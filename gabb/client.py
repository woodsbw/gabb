...
class GabbAuth(requests.auth.AuthBase):
    ...
    def _newAuthentication(self) -> None:
        ...
        resp = requests.post(
            self.auth_url, headers=self._required_headers, data=payload
        )

        self._updateTokensFromResponse(response=resp)

    def _refreshAuthentication(self) -> None:
        """Performs an API token refresh"""
        payload = json.dumps({"refreshToken": self._refresh_token})

        resp = requests.post(
            self.refresh_url, headers=self._required_headers, data=payload
        )

        self._updateTokensFromResponse(response=resp)

    def _updateTokensFromResponse(self, response: requests.Response) -> None:
